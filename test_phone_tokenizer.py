...
""",
    ...
    def test_skips_empty_json_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory) / "repo"
            root.mkdir()
            ...
            data_file.write_text('{"phone": "89992102974"}', encoding="utf-8")
            ...
            self.assertTrue(
                any("Skipping empty JSON file" in message for message in logs.output)
            )
            write_prepared_files(prepared)
            self.assertEqual(empty_file.read_text(encoding="utf-8"), "")
            self.assertEqual(blank_file.read_text(encoding="utf-8"), " \n\t")
            self.assertEqual(
                json.loads(data_file.read_text(encoding="utf-8"))["phone"],
                "88888888888",
            )
